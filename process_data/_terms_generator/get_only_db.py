...
LINK = "https://en.wikipedia.org/wiki/List_of_in-memory_databases"
...
def is_valid_fragment(frag: str) -> bool:
    ...
    if any(f in frag.lower() for f in LOCAL_STOP_FRAGMENT):
        return False
...
def extract_terms(content) -> None:
...
def persist_labels(terms: set[str]) -> None:
    for accepted in sorted(terms):
        print()
        print(accepted.lower())
        db_utils.add_label(accepted.lower(), "DATABASES")
...
def main() -> None:
    ...
    if content is None:
        raise RuntimeError("Could not find content div with id='mw-content-text'")

    clean_content(content)
    extract_terms(content)

    save_terms("ACCEPTED_db.txt", ACCEPTED)
    save_terms("REJECTED_db.txt", REJECTED)

    persist_labels(ACCEPTED)
